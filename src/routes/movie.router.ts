...
router.post('/movies', movieValidation, async (req: Request, res: Response) => {
    ...
    try {
        const newMovie = await MovieModel.create({ ...req.body, title: decodedTitle });
...
router.put('/movies/:title', movieValidation, async (req: Request, res: Response) => {
    const errors = validationResult(req);
...
router.delete('/movies/:title', async (req: Request, res: Response) => {
    const decodedTitle = decodeURIComponent(req.params.title);

    try {
        const deletedMovie = await MovieModel.findOneAndDelete({ title: decodedTitle });
